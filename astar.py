...
def astar(pancake_stack):
	...
	while(not frontier.is_empty()):
		...
		for i in range(len(curr.stack)):
			...
			# Update costs
			flip_copy.g_cost += 1
			flip_copy.h_cost = flip_copy.heuristic()
			flip_copy.f_cost = flip_copy.g_cost + flip_copy.h_cost
			...
			if (flip_copy not in frontier_stacks):
				...
				frontier.push(flip_copy.f_cost, flip_copy)
				frontier_size += 1
...
if __name__=='__main__':
	...
	correct,flips, size = astar(pancake_problem)
